"""
Audio manager — synthesises all sounds in code using numpy so no external
audio files are needed.  If numpy is not installed the game still runs;
all audio calls silently do nothing.
"""
import pygame
import math
...
def _make_tone(freq, duration, volume=0.35, shape='sine'):
    """
    Build a mono waveform array and return a pygame.Sound.
    ...
    # Simple ADSR envelope (short attack + release to avoid clicks)
    env     = np.ones(n)
    ...
    stereo = np.column_stack([mono, mono])   # pygame needs stereo
    return pygame.sndarray.make_sound(stereo)


def _make_ambient(fund_freq, duration=4.0):
    """
    Layered harmonic drone for background music.
    Returns None if numpy is unavailable.
    """
    if not _HAS_NUMPY:
        return None
    ...
    # Fundamental + overtones at decreasing volume
    wave = (
    ...
    # Slow amplitude wobble gives a "breathing" feel
...
class AudioManager:
    ...
    Sounds:
        ...
        'hurt'          — harsh buzz on damage
        'pickup'        — pleasant bell on oxygen collect
        'enemy_death'   — descending buzz on enemy kill
        'level_complete'— rising tone on exit
        'low_oxygen'    — pulsing alert beep

    Music:
        start_music(level)  — looping ambient drone (different per level)
        stop_music()
    """
