...
class Level:
    ...
    def __init__(self, tilemap, level_num):
        ...
        # Generate a deterministic star field (seed = level number)
    ...
    def _parse(self, tilemap):
        for row_idx, row in enumerate(tilemap):
            row = row.ljust(LEVEL_COLS)   # pad to uniform width
            for col_idx, cell in enumerate(row):
                ...
                if cell == 'X':
                    ...
                    # Oxygen canister — slightly smaller than a full tile
                    margin = 8
    ...
    def draw_background(self, surface, offset_x):
        """
        Fill background and draw moon-themed scenery.
        Stars use parallax scrolling (move at 30 % of camera speed).
        """
        ...
        if self.level_num == 1:
            ...
            # Cavern: subtle purple glow rising from the bottom
    ...
    def draw_items(self, surface, offset_x, elapsed):
        ...
        for oxy in self.oxygen_rects:
            ...
            # Pulsing glow ring (special effect — glowing canister)
        ...
        if self.exit_rect:
            ...
            # Expanding glow ring (special effect — portal activation glow)
